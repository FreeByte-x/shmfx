...
- **Tách boundary core/app**: `libshmfx` là framework shared-memory chung; Distributed Logging chuyển sang tài liệu riêng [`03_LOGGING_APP.md`](03_LOGGING_APP.md) và repo path `apps/logging/`.
...
**Changelog 0.2 → 0.3** (fold từ review v0.2, nay lưu trong lịch sử git):
...
**Changelog 0.1 → 0.2** (fold từ review v0.1, nay lưu trong lịch sử git):
...
Reference apps nằm ngoài ba lớp này, ví dụ Distributed Logging ở `apps/logging/` và [`03_LOGGING_APP.md`](03_LOGGING_APP.md).
...
|-----|-----------|--------|---------|
| Distributed Logging | `apps/logging/` | [`03_LOGGING_APP.md`](03_LOGGING_APP.md) | Dùng `SegmentType::RecordRing` + `MpscRing`; optional build target. |
